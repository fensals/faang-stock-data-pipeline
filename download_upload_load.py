...
for filename in os.listdir(dataset):
    ...
    if os.path.isfile(file_path):
        ...
        print(f"Dataset uploaded to GCS: gs://{bucket_name}/faang-stock-dataset/{filename}")
...
for filename in os.listdir(dataset):
    ...
    if os.path.isfile(file_path):
        ...
        table_id = f"{table_id_prefix}{filename.split('.')[0]}"  # Create a table for each CSV file
